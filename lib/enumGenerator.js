...
function generateEnumI18NHtmlFile(swagger, options) {
    ...
    function generateI18NEnumsHtml(enumTypeCollection, outputFileName, options) {
        ...
        // v enum subtype per enum type -> change enumPipe to use this
        // TODO patch values -> 
        /*
           v cancelledContract -> Cancelled Contract
           v reinsurance_contract -> Reinsurance contract

           v _.startCase('--foo-bar--'); => 'Foo Bar'
           v also use for the enumLanguageFiles!! 
           v     so getEnumDefinitions must return the right data already
           v     or call a new function in both generate methods that enriches the values with the startCase labels
           v but leave all uppercase codes intact (country codes)?
        */
        //   Test enumLanguageFiles is empty - what does that do with the existing files
        //   What to do with the country codes ???
        //   and Currencies? (do not use translate pipe??)
        //   test what happens when 1 enum value is removed (or renamed)
        //   build-i18n and test
        //   TODO remove en.json and de.json
        //   TODO upgrade packages (security vulnerability found)
        //   TODO make sure the language file is installed with the PWA
        //   TODO document new generator feature
        //   TODO push new version to Github
        let template = utils.readAndCompileTemplateFile('generate-enum-i18n-html.hbs');
        let result = template(data);
        let isChanged = utils.writeFileIfContentsIsChanged(outputFileName, result);
        if (isChanged) {
            utils.log(`generated ${enumTypeCollection.length}  enums in ${outputFileName}`);
...
function filterEnumDefinitions(enumTypeCollection, node, enumArrayType) {
    _.forEach(node, function (item, key) {
        if (_.isObject(item)) {
            if (item.enum) {
                ...
                filterEnumDefinitions(enumTypeCollection, item, enumArrayType);
            }
        }
    });
}
