...
function getTypeDefinitions(swagger, options, suffix, fileSuffix) {
    ...
    _.forEach(swagger.definitions, (item, key) => {
        let type = getTypeDefinition(typeCollection, item, key, options, suffix, fileSuffix);
...
function getTypeDefinition(typeCollection, item, key, options, suffix, fileSuffix) {
    ...
    _.forEach(properties, (item, key) => {
        let property = getTypePropertyDefinition(type, baseType, required, item, key, options, suffix, fileSuffix)
        type.properties.push(property);
    });
    return type;
}

function getTypePropertyDefinition(type, baseType, required, item, key, options, suffix, fileSuffix) {
    let propertyType = getPropertyType(item, key, options);
    let isRefType = item.$ref;
    let isArray = item.type == 'array';
    let isComplexType = isRefType || isArray; // new this one in constructor
    let isImportType = isRefType || (isArray && item.items.$ref);
    ...
    let validators = getTypePropertyValidatorDefinitions(required, item, key, propertyType.typeName);
    let hasValidation = _.keys(validators.validation).length > 0;

    let isEnum = item.type == 'string' && item.enum;
    let importEnumType = isEnum ? propertyType.typeName : undefined;
...
function getTypePropertyValidatorDefinitions(required, item, key, typeName) {
...
function getPropertyType(item, name, options) {
    let result = {
        typeName: '',
        namespace: '',
    ...
    if (item.type) {
        ...
        if (item.type == 'integer') { result.typeName = 'number' };
        if (item.type == 'string' && item.format == 'date') { result.typeName = 'Date' };
        if (item.type == 'string' && item.format == 'date-time') { result.typeName = 'Date' };
        if (item.type == 'string' && item.enum) { result.typeName = `${name}` };
        if (item.type == 'array' && item.items) {
            let arrayPropType = getPropertyType(item.items, name, options);
            result.typeName = `Array<${arrayPropType.typeName}>`;
            result.namespace = arrayPropType.namespace;
            result.isArrayComplexType = item.items.$ref;
...
function generateTSModels(namespaceGroups, folder, options) {
    ...
    for (let namespace in namespaceGroups) {
        ...
        let firstType = typeCol[0] || { namespace: '' };
...
function removeFilesOfNonExistingTypes(typeCollection, folder, options, suffix) {
    ...
    _.each(files, (file) => {
        if (_.endsWith(file, suffix) && !_.find(typeCollection, (type) => { return type.fileName == file; })) {
            counter++;
